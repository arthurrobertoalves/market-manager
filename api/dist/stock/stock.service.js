...
let StockService = class StockService {
    prisma;
    logger = new common_1.Logger('EstoqueAlerta');
    constructor(prisma) {
        this.prisma = prisma;
    }
    getStatus(product) {
        if (product.stockQuantity <= product.criticalLevel)
            return 'CRITICO';
        if (product.stockQuantity <= product.mediumLevel)
            return 'MEDIO';
        return 'OK';
    }
    async checkStockLevel(product) {
        ...
        this.logger.warn(`[Aviso automático -> fornecedor e gerente] ${message} (produto ${product.id})`);
        return alert;
    }
    findAlerts(resolved) {
